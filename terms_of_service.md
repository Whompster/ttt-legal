# Terms of Service

Last updated: 2026-05-04

Hosted at: https://whompster.github.io/ttt-legal/terms_of_service.html

By downloading, installing, or using the TTT app ("Service") you agree to these terms.
If you do not agree, do not use the Service.

## 1. Account

- You must provide a valid email address to create an account.
- You are responsible for maintaining the confidentiality of your credentials.
- You must not share your account or allow others to access it.
- You must be at least 13 years old (16 in the EEA) to create an account.

## 2. Acceptable use

You agree NOT to:
- Harass, threaten, or abuse other players.
- Use automated scripts, bots, or other tools to interact with the Service.
- Attempt to reverse-engineer, decompile, or tamper with the Service or its servers.
- Exploit bugs or vulnerabilities rather than reporting them responsibly.
- Use the Service for any illegal purpose.

We reserve the right to suspend or terminate accounts that violate these rules,
with or without prior notice.

## 3. Service availability

The Service is provided "as is" without warranty of any kind.
We do not guarantee uptime, availability, or uninterrupted access.
We may modify, suspend, or discontinue the Service at any time.

## 4. Intellectual property

All content, code, and assets in the Service are owned by Håkon Strand Aandahl.
You are granted a limited, non-exclusive, non-transferable licence to use the app
for personal, non-commercial purposes only.

## 5. User-generated content

You retain ownership of any content you create (usernames, in-game chat).
By submitting content to the Service you grant us a worldwide, royalty-free licence
to store and display it to other users as part of the Service.

## 6. Limitation of liability

To the maximum extent permitted by law, we are not liable for any indirect,
incidental, or consequential damages arising from your use of the Service,
including loss of data or account access.

## 7. Termination

We may terminate your account at any time for breach of these terms.
You may delete your account at any time through the app.

## 8. Governing law

These terms are governed by the laws of Norway.
Any disputes shall be resolved in the courts of Norway.

## 9. Changes

We may update these terms from time to time. Continued use after a change
constitutes acceptance of the updated terms.

## 10. Contact

whompster101+legal@gmail.com
